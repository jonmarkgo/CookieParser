...
Format rules:
- If any recipes are fundamentally different variants (e.g., oatmeal cookies vs chocolate chip cookies, banana bread vs zucchini bread), START with a ## Quick Take section (1-2 bullets) that calls this out plainly — what makes them different kinds of ${typeLabel}, not just ratio differences.
- Then write 3-5 sections using ## headings. Choose the MOST INTERESTING themes for these specific recipes. Don't use generic headings — tailor them. Examples of possible themes (pick what matters most):
  - Texture differences (spread, chew, crumb)
  - Fat strategy (amount, type, melted vs creamed)
  - Sugar balance (sweetness, browning, moisture)
  - Hydration and moisture
  - Egg's role (structure, richness, leavening)
  - Mixing method (cream, melt, reverse-cream, fold)
  - Rest/chill time and why it matters
  - Oven temperature and bake time
  - Unusual or standout technique (e.g., browned butter, aged dough, bloomed cocoa)
  - Tenderizer vs toughener balance (when there's a notable skew)
  - Flavor and add-ins
  - Leavening strategy
- Do NOT use all of these — pick 3-5 that highlight the most meaningful differences between THESE recipes.
- Under each heading, write 2-3 bullet points MAX.
- Each bullet: ONE short sentence (under 20 words). Be punchy and specific.
- Use **bold** for key terms. Include specific numbers (ratios, percentages, times, temperatures).
